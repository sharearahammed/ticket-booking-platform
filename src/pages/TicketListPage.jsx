...
const TicketListPage = ({ category }) => {
  ...
  const { data, fetchNextPage, hasNextPage, isFetchingNextPage } = useInfiniteTickets(category, priceFilter);
            ...
            <div className="mt-6 p-6 sm:p-8 bg-white rounded-2xl border-2 border-blue-200 shadow-xl space-y-6 animate-fadeIn">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {[
                  ...
                  <label key={filter.value} className="flex items-start gap-3 cursor-pointer p-4 rounded-xl hover:bg-gradient-to-br hover:from-blue-50 hover:to-emerald-50 transition-all border-2 border-gray-100 hover:border-blue-300 group">
                    <input
                      type="radio"
                      name="price"
                      value={filter.value}
                    ...
                    <div className="flex-1">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="font-semibold text-gray-900 group-hover:text-blue-600 transition-colors text-sm sm:text-base md:text-lg">{filter.label}</span>
                      </div>
                      <span className="text-xs sm:text-sm text-gray-500 group-hover:text-gray-700 transition-colors ml-7">{filter.desc}</span>
                    </div>
                  </label>
                ))}
              </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {sortedTickets.length > 0 ? (
            sortedTickets.map((ticket) => (
              <TicketCard key={ticket.id} ticket={ticket} onSelect={() => handleSelectTicket(ticket)} />
            ))
          ) : (
